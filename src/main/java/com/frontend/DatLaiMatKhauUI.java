...
public class DatLaiMatKhauUI {
    ...
    public void initialize() {
        ...
        updateHyperlinkIcon(xemMKMoiHyperlink, viewIcon);
        updateHyperlinkIcon(xemXacNhanMKMoiHyperlink, viewIcon);
    }
    ...
    private void xemMatKhauMoi() {
        isMkMoiVisible = !isMkMoiVisible;
        togglePasswordVisibility(mkMoiPWField, hienMKMoiTextField, isMkMoiVisible, xemMKMoiHyperlink);
    }

    @FXML
    private void xemXacNhanMatKhauMoi() {
        isXacNhanMkMoiVisible = !isXacNhanMkMoiVisible;
        togglePasswordVisibility(xacNhanMKMoiPWField, xacNhanMKMoiTextField, isXacNhanMkMoiVisible, xemXacNhanMKMoiHyperlink);
    }

    private void togglePasswordVisibility(PasswordField passwordField, TextField textField, boolean isVisible, Hyperlink hyperlink) {
        if (isVisible) {
            ...
            updateHyperlinkIcon(hyperlink, hideIcon);
        } else {
            ...
            updateHyperlinkIcon(hyperlink, viewIcon);
        }
    }

    private void updateHyperlinkIcon(Hyperlink hyperlink, Image icon) {
        if (hyperlink != null && hyperlink.getGraphic() instanceof ImageView && icon != null) {
            ((ImageView) hyperlink.getGraphic()).setImage(icon);
        } else if (hyperlink != null && icon != null) {
            // Nếu chưa có ImageView, tạo mới (cần thiết nếu FXML không khai báo graphic ban đầu)
            ImageView imageView = new ImageView(icon);
            imageView.setFitHeight(25); // Kích thước mong muốn
            imageView.setFitWidth(25);  // Kích thước mong muốn
    ...
    // @FXML
    // private void handleQuayLaiDatLai() { // Nếu có nút quay lại
    //     closeDialog();
    // }
